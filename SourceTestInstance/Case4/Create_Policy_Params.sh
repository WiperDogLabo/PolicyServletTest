...
contentPost=$(curl -X POST -H "Accept: application/json" -H "Content-type: application/json" -d '{"action":"WRITE2FILE",
	"jobName":"MySQL.Database_Area.JobTest01","instanceName":"inst2-@MYSQL-information_schema",
	"params" : {"MaxIOTime":"30"},
	"policyStr":"POLICY = {
		resultData->\n 
		def listMess = []\n 
		def ret = [\"jobName\" : \"MySQL.Database_Area.JobTest01\", \"istIid\" : \"inst2-@MYSQL-information_schema\"]\n 
		resultData.each{data->\n 
...
if [[ $contentPost =~ .*'status'.* ]]; then
	echo "[MESSAGE TEST] POST IS OK"
	echo "CHECK IN FOLDER VAR/JOB/POLICY: MySQL.Database_Area.JobTest01.inst2-@MYSQL-information_schema.policy and MySQL.Database_Area.JobTest01.inst2-@MYSQL-information_schema.params WAS CREATED !!!"
else
	echo "[MESSAGE TEST] GET IS FAILURE, SO CANNOT TEST POST METHOD"
fi
