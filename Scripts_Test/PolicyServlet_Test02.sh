...
contentPost=$(curl -X POST -H "Accept: application/json" -H "Content-type: application/json" -d '{"action":"WRITE2FILE",
	"jobName":"MySQL.Database_Area.Top30Database","instanceName":"master",
	"params" : {},
	"policyStr":"POLICY = {
		resultData->\n 
		def listMess = []\n 
		def ret = ['jobName' : 'MySQL.Database_Area.Top30Database', 'istIid' : 'master']\n 
		resultData.each{data->\n 
			if((data.UsedPct < 1)){\n 
				listMess.add([level: 2, message: \"\"\"Message test for policyServlet02!!!\"\"\"])\n }\n }\n 
...
echo "*NOTE: CHECK IN FOLDER VAR/JOB/POLICY: MySQL.Database_Area.Top30Database.master.policy WAS CREATED !!!"
fi
else
