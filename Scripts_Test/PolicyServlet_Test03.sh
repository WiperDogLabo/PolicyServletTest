...
contentPost=$(curl -X POST -H "Accept: application/json" -H "Content-type: application/json" -d '{"action":"WRITE2FILE",
	"jobName":"MySQL.Database_Area.TestServlet03","instanceName":"master",
	"params" : {"DatabaseName":"db_test","Max_Write":"1000"},
	"policyStr":"POLICY = {
		resultData->\n 
		def listMess = []\n 
		def ret = ['jobName' : 'MySQL.Database_Area.TestServlet03', 'istIid' : 'master']\n 
...
echo "*NOTE: CHECK IN FOLDER VAR/JOB/POLICY: MySQL.Database_Area.TestServlet03.master.policy WAS CREATED !!!"
fi
else
