...
echo ">>>>> TEST POST METHOD OF POLICY SERVLET (params is empty)<<<<<"
contentPost=$(curl -X POST -H "Accept: application/json" -H "Content-type: application/json" -d '{"action":"WRITE2FILE",
	"jobName":"MySQL.Database_Area.InnoDBTablespace_Free","instanceName":"",
	"params" : {},
	"policyStr":"POLICY = {
		resultData->\n 
		def listMess = []\n 
		def ret = ['jobName' : 'MySQL.Database_Area.InnoDBTablespace_Free', 'istIid' : '']\n 
...
echo "*NOTE: CHECK IN FOLDER VAR/JOB/POLICY: MySQL.Database_Area.InnoDBTablespace_Free.policy WAS CREATED !!!"
fi
else
